import {Component} from '@angular/core';
import {EmployeeService} from "../services/employee.service";
import {Employee} from "../services/employee";
import {HrResponse} from "../services/hrresponse";

@Component({
  selector: 'app-root',
  templateUrl: './app.component.html',
  styleUrls: ['./app.component.css'],
  providers: [EmployeeService]
})
export class AppComponent {
  private identityNo: string;
  private fullname: string;
  private iban: string;
  private salary: number;
  private birthYear: number;
  private department: string;
  private photo: string;
  private employees: Employee[];

  constructor(private employeeService: EmployeeService) {
    this.identityNo = "64802693648";
    this.fullname = "Jack Bauer";
    this.iban = "TR310267702240447648902639";
    this.salary = 10000;
    this.birthYear = 1986;
    this.department = "Finance";
    this.photo = "data:image/png;base64,";
    this.employees = [];
  }

  get averageSalary() {
    if (this.employees.length == 0) return "N/A";
    return this.employees.map(emp => emp.salary).reduce((avg, sal) => avg + sal / this.employees.length, 0);
  }

  findAllEmployees() {
    this.employeeService.findAllEmployees()
      .subscribe(employees => {
        this.employees = employees;
      });
  }

  copyEmployee(emp) {
    this.fullname = emp.fullname;
    this.salary = emp.salary;
    this.iban = emp.iban;
    this.photo = emp.photo;
    this.birthYear = emp.birthYear;
    this.department = emp.department;
  }

  findEmployee() {
    this.employeeService.findEmployee(this.identityNo)
      .subscribe(employee => {
        this.fullname = employee.fullname;
        this.salary = employee.salary;
        this.iban = employee.iban;
        this.photo = employee.photo;
        this.birthYear = employee.birthYear;
        this.department = employee.department;
      });
  }

  addEmployee() {
    this.employeeService.addEmployee(new Employee(this.identityNo, this.fullname, this.iban, this.salary, this.birthYear, this.department, this.photo))
      .subscribe((response: HrResponse) => {
      });
  }

  updateEmployee() {
    this.employeeService.updateEmployee(new Employee(this.identityNo, this.fullname, this.iban, this.salary, this.birthYear, this.department, this.photo))
      .subscribe((response: HrResponse) => {
      });
  }

  removeRow(empToBeRemoved: Employee) {
    this.employeeService.removeEmployee(empToBeRemoved.identityNo)
      .subscribe((employee: Employee) => {
        this.fullname = employee.fullname;
        this.salary = employee.salary;
        this.iban = employee.iban;
        this.photo = employee.photo;
        this.birthYear = employee.birthYear;
        this.department = employee.department;
        this.employees = this.employees.filter(emp => emp != empToBeRemoved);
      });
  }

  removeEmployee() {
    this.employeeService.removeEmployee(this.identityNo)
      .subscribe((employee: Employee) => {
        this.fullname = employee.fullname;
        this.salary = employee.salary;
        this.iban = employee.iban;
        this.photo = employee.photo;
        this.birthYear = employee.birthYear;
        this.department = employee.department;
      });
  }
}
