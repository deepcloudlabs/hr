    ...
    let app = new Vue({
        ...
        computed: {
            averageSalary: function () {
                ...
                let totalSalary =
                    this.employees.reduce((sum, emp) => emp.salary + sum, 0);
                return Number.parseFloat(totalSalary / this.employees.length).toFixed(2);
            }
        },
        created: function () {
        },
        methods: {
            ...
            deleteRow: function (emp, index) {
                jQuery.ajax({
                    method: "DELETE",
                    url: "http://localhost:7001/employees/" + emp.identityNo,
                    success: (employee) => {
                        ...
                        toastr.success("Employee is removed!");
                    }
                });
            },
            addEmployee: function () {
                $.ajax({
                    method: "POST",
                    url: "http://localhost:7001/employees",
            ...
            removeEmployee: function () {
                jQuery.ajax({
                    method: "DELETE",
                    url: "http://localhost:7001/employees/" + this.identityNo,
                    success: (employee) => {
                        this.fullname = employee.fullname;
                        this.birthYear = employee.birthYear;
                        this.salary = employee.salary;
